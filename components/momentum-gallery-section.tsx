...
export function MomentumGallerySection() {
  ...
  useEffect(() => {
    ...
    function setTitle(idx: number) {
      titlesEl.innerHTML = `<span>${slides[idx].title}</span>`
    }
    ...
    async function init() {
        ...
        slides.map(
          (s) =>
            new Promise<THREE.Texture>((resolve) => {
              texLoader!.load(s.src, (t: THREE.Texture) => resolve(t))
            })
        )
      )
      ...
      const mat = new THREE.ShaderMaterial({
        uniforms: {
          uTex0: { value: textures[state.current] },
          uTex1: { value: textures[state.next] },
    ...
    return () => {
      ;(async () => (await cleanupPromise) && (await cleanupPromise)())()
    }
  }, [])
